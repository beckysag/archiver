...
/* Quickly append all regular files in the current directory (except
 * the archive itself  */
void append_all(int fd,char*ar_name){
	...
	while ((dp = readdir(dirp)) != NULL) { /* Get file count*/
		if ((stat(dp->d_name, &st))==-1){
			printf("Couldn't statvcurrent directory.\n");
			exit(EXIT_FAILURE);
		}
	    if ((S_ISREG(st.st_mode)) != 0) { /* If it's a regular file */
	         file_count++;
	    }
	}
	list = malloc(file_count * sizeof(char*)); /* allocate memory to store filenames */
	rewinddir(dirp);
	...
	for (;;) {
		...
		if ((dp = readdir(dirp)) == NULL)
			break;
		if (strcmp(dp->d_name, ".") == 0 	/* Skip '.' '..' and ar_name */
				|| strcmp(dp->d_name, "..") == 0
				|| strcmp(dp->d_name, ar_name) == 0)
			continue;
		if ((S_ISREG(st.st_mode)) != 0) { /* If it's a regular file */
			list[list_size] = (char*)malloc(strlen(dp->d_name) * sizeof(char));
			strcpy(list[list_size], dp->d_name);
			list_size++;
	    }
	}

	if (errno != 0) {
		printf("Error reading contents of directory.%s\n,", strerror(errno));
		exit(EXIT_FAILURE);
	}
	append(fd, list, list_size); /* quick append files in list */

	for (i = 0; i < list_size; i++) /* Free malloc'd memory */
		free(list[i]);
	free(list);

	if (closedir(dirp) == -1) {
		printf("Error while trying to close directory\n");
		exit(EXIT_FAILURE);
	}
}
...
void delete(int *fd_old, char*arname, char**file_list, int n) {
	...
	if ((fd_new = open(arname, O_RDWR | O_CREAT | O_TRUNC, st.st_mode)) == -1) {
		printf("Error creating archive archive file.\n");
		exit(EXIT_FAILURE);
	}
	...
	while ((long)ar_pos < (long)ar_size) {
		...
		if ((ar_head.ar_name[1] != ' ') && (ar_head.ar_name[1] != '/')) {
			...
			if (chmod(arf_name, mode) == -1) {
				printf("Error: could not set file permissions\n");
				exit(-1);
			}
...
void usage() {
	printf("Usage: ar [-]{qxtvdAw} [member-name] archive-file file...\n"
			" commands:\n"
			...
			"  A            - append all regular files in current directory\n");
}
